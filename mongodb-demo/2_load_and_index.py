...
documents = reader.load_data(
    ...
    os.getenv(
        "MONGODB_COLLECTION"
    ),  # this is the collection where the objects you loaded in 1_import got stored
    field_names=[
        "full_text"
    ],  # these is a list of the top-level fields in your objects that will be indexed
...
store = MongoDBAtlasVectorSearch(
    ...
    collection_name=os.getenv(
        "MONGODB_VECTORS"
    ),  # this is where your embeddings will be stored
    index_name=os.getenv(
        "MONGODB_VECTOR_INDEX"
    ),  # this is the name of the index you will need to create
)
